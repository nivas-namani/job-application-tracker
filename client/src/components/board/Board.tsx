...
import { Archive, Clock3, Search, Upload } from 'lucide-react';
import { useMemo, useState } from 'react';
import { getMetrics } from '../../lib/application-utils';
import type { Application, ApplicationStatus, HiringProcess } from '../../types';
...
export function Board({ applications, processes, onSelect, onAdd, onStatusChange, onOpenTransfer }: BoardProps) {
  ...
  const liveApplications = useMemo(() => showArchived ? applications : applications.filter((application) => !application.archivedAt), [applications, showArchived]);
  const metrics = getMetrics(liveApplications.filter((application) => !application.archivedAt));
  const stageNames = useMemo(() => new Map(processes.flatMap((process) => process.stages.map((stage) => [stage.id, stage.name] as const))), [processes]);

  const visibleApplications = useMemo(() => liveApplications.filter((application) => {
  ...
  }).sort((a, b) => sort === 'newest' ? new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime() : new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()), [liveApplications, query, filter, sort]);
  ...
  const archivedCount = applications.filter((application) => application.archivedAt).length;

  return <div className="page-shell"><section className="mb-6"><h1 className="text-2xl font-extrabold tracking-tight sm:text-[28px]">Your search</h1><p className="mt-1 text-sm text-muted">{metrics.total} applications · {metrics.active} still active{archivedCount > 0 && ` · ${archivedCount} archived`}</p></section><section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label="Active" value={metrics.active.toString()} note={`of ${metrics.total} tracked`} /><MetricCard label="Response rate" value={`${metrics.responseRate}%`} note={`${metrics.replied} of ${metrics.applied} replied`} /><MetricCard label="Median reply time" value={metrics.medianReplyDays === null ? '—' : `${metrics.medianReplyDays}d`} note="applied to first reply" /><MetricCard label="Needs follow-up" value={metrics.needsFollowUp.toString()} note="quiet for 14+ days" alert={metrics.needsFollowUp > 0} /></section><section className="mt-6 flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative w-full lg:max-w-[322px]"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search company, role or location" className="form-input py-2.5 pl-10 pr-4" /></div><div className="flex flex-wrap gap-2"><FilterButton active={filter === 'ALL'} onClick={() => setFilter('ALL')}>All</FilterButton><FilterButton active={filter === 'FOLLOW_UP'} onClick={() => setFilter('FOLLOW_UP')}><Clock3 className="h-3.5 w-3.5" />Needs follow-up</FilterButton><FilterButton active={filter === 'REFERRALS'} onClick={() => setFilter('REFERRALS')}>Referrals</FilterButton><FilterButton active={showArchived} onClick={() => setShowArchived((current) => !current)}><Archive className="h-3.5 w-3.5" />Archived{archivedCount > 0 && ` (${archivedCount})`}</FilterButton></div><div className="ml-auto flex items-center gap-2"><button onClick={onOpenTransfer} className="button-secondary py-2"><Upload className="h-4 w-4" />Import / export</button><label className="inline-flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm text-muted shadow-sm">Sort<select aria-label="Sort applications" value={sort} onChange={(event) => setSort(event.target.value as 'newest' | 'oldest')} className="bg-transparent font-semibold text-ink outline-none"><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label></div></section><DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => setActiveDrag(null)}><section className="board-lane" aria-label="Application board">{BOARD_STATUSES.map((status) => <BoardColumn key={status} status={status} applications={visibleApplications.filter((application) => application.status === status)} stageNames={stageNames} onSelect={onSelect} onAdd={onAdd} />)}<ClosedColumn applications={visibleApplications.filter((application) => CLOSED_STATUSES.includes(application.status))} onSelect={onSelect} showingArchived={showArchived} /></section><DragOverlay dropAnimation={null}>{activeDrag ? <ApplicationCard application={activeDrag} onSelect={() => undefined} overlay stageLabel={activeDrag.processStageId ? stageNames.get(activeDrag.processStageId) ?? null : null} /> : null}</DragOverlay></DndContext></div>;
}
